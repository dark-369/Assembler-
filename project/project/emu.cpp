...
void ldc(int opr,int &program_counter)
{
    reg2 = reg1;
    reg1 = opr;
}

void adc(int opr,int &program_counter)
{
    reg1 += opr;
}

void ldl(int opr,int &program_counter)
{
    reg2 = reg1;
    reg1 = memory_tab[stack_pointer + opr];
}

void stl(int opr,int &program_counter)
{
    memory_tab[stack_pointer + opr] = reg1;
    reg1 = reg2;
}

void ldnl(int opr,int &program_counter)
{
    reg1 = memory_tab[reg1 + opr];
}

void stnl(int opr,int &program_counter)
{
    memory_tab[reg1 + opr] = reg2;
}

void add(int &program_counter)
{
    reg1 += reg2;
}

void sub(int &program_counter)
{
    reg1 = reg2 - reg1;
}

void shl(int &program_counter)
{
    reg1 = reg2 << reg1;
}

void shr(int &program_counter)
{
    reg1 = reg2 >> reg1;
}

void adj(int opr,int &program_counter)
{
    stack_pointer += opr;
}

void a2sp(int &program_counter)
{
    stack_pointer = reg1;
    reg1 = reg2;
}

void sp2a(int &program_counter)
{
    reg2 = reg1;
    reg1 = stack_pointer;
}

void call(int opr,int &program_counter)
{
    reg2 = reg1;
...
void returnInstruction(int opr, bool &when_to_halt,int &program_counter)
{
...
void brz(int opr,int &program_counter)
{
    if (reg1 == 0)
        program_counter += opr;
}

void brlz(int opr,int &program_counter)
{
    if (reg1 < 0)
        program_counter += opr;
}

void br(int opr,int &program_counter)
{
    program_counter += opr;
}

void HALT(bool &when_to_halt,int &program_counter)
{
    when_to_halt = true;
}
...
void monitor_flow(ofstream &tf, int program_counter)
{
    ...
    for (;;)
    {
        ...
        if (op_code == 0)
        {
            ldc(opr,program_counter);
        }
        else if (op_code == 1)
        {
            adc(opr,program_counter);
        }
        else if (op_code == 2)
        {
            ldl(opr,program_counter);
        }
        else if (op_code == 3)
        {
            stl(opr,program_counter);
        }
        else if (op_code == 4)
        {
            ldnl(opr,program_counter);
        }
        else if (op_code == 5)
        {
            stnl(opr,program_counter);
        }
        else if (op_code == 6)
        {
            add(program_counter);
        ...
        {
            adj(opr,program_counter);
        }
        else if (op_code == 11)
        {
            a2sp(program_counter);
        }
        else if (op_code == 12)
        {
            sp2a(program_counter);
        }
        else if (op_code == 13)
        {
            call(opr,program_counter);
        }
        else if (op_code == 14)
        {
            returnInstruction(opr, when_to_halt,program_counter);
        }
        else if (op_code == 15)
        {
            brz(opr,program_counter);
        }
        else if (op_code == 16)
        {
            brlz(opr,program_counter);
        }
        else if (op_code == 17)
        {
            br(opr,program_counter);
        }
        else if (op_code == 18)
        {
            HALT(when_to_halt,program_counter);
        }
