...
vector<pair<int,int>> Second_Pass(){
    ...
    while(i<store_instructions.size()){
        ...
        int operand_int = 0;
        if(opcodemapping[instruction].first > 0) operand_int = operand_processor(operand, l);
...
//    cout << error_list.size() << endl;
    ...
    else
    {
       ...
       // Those who take offsets, here have to find the distance
        set<int> offset_calc;
        offset_calc.insert(13); offset_calc.insert(15);
        offset_calc.insert(16); offset_calc.insert(17);
        ...
        obptr << hexadecimal_conversion(operand, 6) << hexadecimal_conversion(Opcode, 2) << endl;
        i++;
       }
...
bool binary_check(string str){
    ...
    if (str.empty() || (str.size() > 1 && str.substr(0, 2) != "0b"))
        return false;
...
bool is_valid_lable(string s){
    int n = s.size();
    if(!((s[0] <= 'Z' && s[0] >= 'A')||(s[0] <= 'z' && s[0] >= 'a'))) return false;
    for(int i = 1; i < n; i ++){
       if(!((s[0] <= 'Z' && s[0] >= 'A')||(s[0] <= 'z' && s[0] >= 'a')||(s[0] <= '9' && s[0] >= '0'))) return false;
    }
    return true;
}
